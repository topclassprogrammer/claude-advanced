...
src/
  ...
  summary/
    ...
    anthropic-summary.service.ts          — реализация поверх `@anthropic-ai/sdk` (`client.messages.parse` + `output_config.format` из `zodOutputFormat`, модель `claude-opus-5`) — structured output с summary/actionItems (assignee — только если назван в транскрипте)/decisions; клиент создаётся лениво (в поле, не при бутстрапе), чтобы отсутствие `ANTHROPIC_API_KEY` не ронял приложение — ошибка авторизации всплывает только при первом вызове `generateSummary`
    commands/impl/start-meeting-file-summary.command.ts, commands/handlers/start-meeting-file-summary.handler.ts — создаёт запись `MeetingFileSummary` со статусом PENDING (запускается после того, как транскрипт файла перешёл в COMPLETED — не запускается для FAILED-транскриптов), затем без ожидания (fire-and-forget, ошибки логируются) отправляет `ProcessMeetingFileSummaryCommand`; если файл встречи уже удалён конкурентно (FK-constraint при create), тихо возвращает null вместо ошибки
    commands/impl/process-meeting-file-summary.command.ts, commands/handlers/process-meeting-file-summary.handler.ts — переводит статус в PROCESSING, вызывает `ClaudeSummaryService.generateSummary`, затем COMPLETED (summary/decisions/action items) или FAILED (с `errorMessage`, не отдаётся клиенту); статус обновляется через `updateMany` (не `update`), action items досоздаются `createMany` только если запись выжимки всё ещё существует — конкурентное удаление файла не приводит к необработанной ошибке
    anthropic-summary.service.spec.ts       — unit-тест `AnthropicSummaryService` с замоканным `@anthropic-ai/sdk` (успешный разбор, ответ не прошёл валидацию схемы, ошибка вызова API)
test/
