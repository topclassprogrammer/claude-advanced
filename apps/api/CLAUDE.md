...
Бэкенд на NestJS 11 (TypeScript), персистентность — PostgreSQL через Prisma ORM 7. Модули: `UsersModule`, `AuthModule` (JWT), `MeetingModule`, `MeetingFileModule`, `ProfileModule`, `TranscriptionModule` — детали по каждому и его эндпоинтам см. в «Структуре» ниже. Бизнес-логика организована по паттерну CQRS (`@nestjs/cqrs`, см. раздел «CQRS»); межмодульное взаимодействие — только через `CommandBus`/`QueryBus`, без прямых импортов хендлеров/провайдеров другого модуля.
...
```
prisma/
  schema.prisma          — схема БД (модели User, Meeting — с необязательным полем description, MeetingFile, MeetingFileTranscription — один-к-одному с MeetingFile, onDelete: Cascade, enum TranscriptionStatus, RefreshToken) и generator/datasource
...
src/
  ...
  meeting-file/
    ...
    queries/impl/get-meeting-files.query.ts, queries/handlers/get-meeting-files.handler.ts — проверяет, что встреча существует и запрашивающий — организатор (иначе 404/403), затем список `MeetingFile` по `meetingId` вместе со связанной `transcription` (Prisma `include`, без похода в `TranscriptionModule` через `QueryBus` — это чтение через общую БД, а не бизнес-логика другого модуля), отсортированный по `uploadedAt` по убыванию (самые новые первыми); пустой список, если файлов нет
    meeting-file.types.ts                     — `MeetingFileRecord` (публичная форма `MeetingFile`, без `storagePath`) дополнительно содержит `transcription: MeetingFileTranscriptionRecord | null` (status/text из `transcription/transcription.types.ts`, `null` для файлов без транскрипта — например, не mp4/mp3)
  profile/
