...
- `/profile` (`src/app/profile/page.tsx`) — требует авторизации. Аватар/имя/email через `GET /users/me`. Только отображение — редактирования имени/пароля/аватара в UI нет, хотя API это поддерживает (`PATCH /users/me/name`, `PATCH /users/me/password`, `POST`/`DELETE /users/me/avatar`, см. `apps/api/CLAUDE.md`, модуль `profile/`).
