...
src/components/
  ...
  FileCard.tsx               — карточка файла встречи (иконка по типу/имя/размер/дата/кнопка «Скачать») + пустое состояние на EmptyState, если файл не загружен
...
src/lib/
  ...
  meeting-file-api.ts        — клиент для GET /meetings/:id/file (метаданные, fetch), GET /meetings/:id/file/download (скачивание через blob, fetch) и POST /meetings/:id/file (загрузка через XMLHttpRequest с прогрессом)
