...
- `/profile` (`src/app/profile/page.tsx`) — требует авторизации. Аватар/имя/email через `GET /users/me`; форма редактирования имени (`ProfileNameForm`, `PATCH /users/me/name`), загрузка/замена/удаление аватара (`AvatarUpload`, `POST`/`DELETE /users/me/avatar`) и форма смены пароля (`ProfilePasswordForm`, `PATCH /users/me/password`).
...
src/components/
  ...
  ProfilePasswordForm.tsx      — форма смены пароля на /profile (текущий/новый пароль, переключатель видимости через EyeIcon/EyeOffIcon), вызывает changePassword (PATCH /users/me/password); при успехе — Alert-подтверждение и сброс полей (ссылка на элемент формы сохраняется в переменную до await — event.currentTarget становится null после точки await в React), неверный текущий пароль (401) показывается как «Неверный текущий пароль.» без потери введённых значений
...
e2e/
  ...
  profile-password.spec.ts     — Playwright e2e-тесты смены пароля: успешная смена с верным старым паролем и последующий логин новым паролем, отказ при неверном старом пароле (понятная ошибка, форма не сбрасывается), отказ при слишком коротком новом пароле (валидационная ошибка)
